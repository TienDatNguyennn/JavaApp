...
public class InvoiceIssuePanel extends JPanel {
    ...
    private void issueInvoice() {
        int idx = cmbIssueInvoice.getSelectedIndex();
        if (paidInvoices == null || paidInvoices.isEmpty() || idx < 0 || idx >= paidInvoices.size()) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn hóa đơn cần phát hành.",
                    "Cảnh báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
        String buyer = txtBuyerName.getText().trim();
        if (buyer.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Vui lòng nhập tên người mua.",
                    "Cảnh báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        inv.setApiStatus("SENT");   // cập nhật tạm để email hiện đúng trạng thái

        loadPaidInvoices();
        loadHistory();

        String toEmail = txtEmail.getText().trim();
        boolean hasEmail = !toEmail.isEmpty() && toEmail.contains("@");

        // Thông báo ngay, không đợi email gửi xong
        String msg = "✔ Đã phát hành hóa đơn điện tử!\n"
                   + "Hóa đơn: INV-" + String.format("%03d", inv.getInvoiceId()) + "\n"
                   + "Người mua: " + buyer + "\n"
                   + (hasEmail
                       ? "Email: đang gửi đến " + toEmail + "..."
                       : "Email: (không có địa chỉ nhận, bỏ qua)");
        JOptionPane.showMessageDialog(this, msg, "Thành công", JOptionPane.INFORMATION_MESSAGE);

        // ── Gửi email trong background nếu có địa chỉ ────────────────
        if (!hasEmail) return;

        final String finalBuyer   = buyer;
        final String finalTax     = txtTaxCode.getText().trim();
        final String finalAddress = txtAddress.getText().trim();
        final String finalType    = cmbInvoiceType.getSelectedItem().toString();
        final Invoice finalInv    = inv;

        new Thread(() -> {
            try {
                ByteArrayOutputStream bos = new ByteArrayOutputStream();
                InvoicePdfGenerator.exportToStream(
                        finalInv, finalBuyer, finalTax, finalAddress, toEmail, finalType, bos);
                EmailService.sendInvoice(toEmail, finalBuyer, finalInv, bos.toByteArray());
                ...
                SwingUtilities.invokeLater(() ->
                    JOptionPane.showMessageDialog(InvoiceIssuePanel.this,
                            "⚠ Phát hành thành công nhưng gửi email thất bại:\n" + ex.getMessage()
                            + "\n\nKiểm tra lại App Password trong ⚙ Cài đặt Gmail.",
